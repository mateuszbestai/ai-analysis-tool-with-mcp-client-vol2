...
class sql_agent():
    def __init__(self):
        ...
        def generate_query(state):
            ...
            query_gen_system = query_gen_system + "\n Based on the following information about table strucure: " + state["messages"][-1].content + "generate an sql query using this prompt: " + state["messages"][0].content
        ...
        def should_generate(state) -> Literal["generate_query", "generate_output"]:
            prompt = "Data: " + state["messages"][-1].content + "\nPrompt:" + state["messages"][0].content
            llm = initialize_llm()
        ...
        def generate_output_query(state):
            prompt = "This sql query "+ str(state["messages"][-2].content) + " returned this sql response: " + str(state["messages"][-1].content) + ". Based on that data and this question: " + state["messages"][0].content + "generate a response"
            print("=====")
            print(prompt)
            print("=====")
            llm = initialize_llm()
        ...
        def improve_query(state):
            ...
            error_msg = "The query returned no rows. "if not result_query else "The query generated the following error: " + result_query
            print("error",error_msg)
            query_gen_system = QUERY_GEN_SYSTEM
            query_gen_system = query_gen_system + "\n The previous query you generated was incorrect: " + state["messages"][-1].content + error_msg +" fix the error and generate a new sql query using this prompt: " + state["messages"][0].content
            llm = initialize_llm()
            structured_llm=llm.with_structured_output(json_schema_generate)
        ...
        model_get_schema = initialize_llm().bind_tools(
            [get_schema_tool]
        )
        workflow.add_node(
            "model_get_schema",
...
if __name__ == "__main__":
    sql = sql_agent()
    idk = {"messages": [("user", "cześć jakie są dni które zamieniły się na not ok a dzien przed tem był ok? użyj danych")]}
